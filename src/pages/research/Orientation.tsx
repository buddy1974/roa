...
// documents.json is already in the main bundle (shared by Archive, DocumentView…)
...
export default function Orientation() {
  ...
  // Lazy-load FAQ data — this becomes a separate Vite chunk
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* Loading skeleton while FAQ chunk loads */}
      {!faqLoaded ? (
        <div className="space-y-3 max-w-xl" aria-hidden="true">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-7 bg-slate-100" />
          ))}
        </div>
      ) : (
        <>
            ...
            <div className="flex-1 min-w-0">
              {selected === null ? (
                ...
                <article key={selected.id} aria-labelledby="entry-question">
                  ...
                  <details key={`deep-${selected.id}`} className="mb-8">
